...
//标准调用方法
void MD5_standarCal(char *buffer,int length,unsigned char *md5)
{
	MD5_CTX ctx;
	MD5_Init(&ctx);
...
void MD5_simpleCal(char *buffer,unsigned long length,unsigned char *md5)
{
	MD5((const unsigned char *)buffer,length,md5);
}

//EVP 接口方法
void MD5_evpCal(char *buffer,unsigned long length,unsigned char *md5)
{
	EVP_MD_CTX ctx;
...
//BIO调用方式
void MD5_BIOCal(char *buffer,unsigned long length,unsigned char *md5)
{
	BIO* bio_null;
	BIO* bio_md;
...
int CalFileMD5(char *filename,char *md5)  
{  
	MD5_CTX ctx;  
	int len = 0;  
	int flen = 0;
...
int CalFileMD5_with_offset(char *filename,int offset,char *md5)  
{  
	MD5_CTX ctx;  
	int len = 0;  
	int flen = 0;
